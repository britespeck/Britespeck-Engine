...
// 1. API Handlers - Use "public." to avoid search_path issues
async fn get_predictions(State(pool): State<sqlx::PgPool>) -> Json<Vec<PredictionEvent>> {
    let rows = sqlx::query_as::<_, PredictionEvent>(
        "SELECT id, title, platform, odds, category, status, icon_url, external_id, volume_24h, updated_at, outcomes, market_url, end_date, rsi_signal, sentiment_score FROM public.prediction_events ORDER BY updated_at DESC LIMIT 5000"
    )
    .fetch_all(&pool)
    .await
    .unwrap_or_default();
